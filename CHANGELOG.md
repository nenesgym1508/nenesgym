...
- **Eliminación de Sistema QR e Ingreso Directo**: Se eliminó por completo la validación de asistencia mediante códigos QR por considerarse un flujo con fricción innecesaria (sin torniquete físico). Se reemplazó por un botón de ingreso directo para clientes y un flujo de búsqueda directa simplificado para administración. Las dependencias `html5-qrcode` y `qrcode` fueron desinstaladas del bundle de forma definitiva.
