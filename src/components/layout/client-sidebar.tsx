...
export function ClientSidebar({ fullName }: ClientSidebarProps) {
  ...
  return (
    <aside className="hidden md:flex md:flex-col fixed left-0 top-0 h-screen w-64 shrink-0 border-r border-white/10 bg-zinc-950 px-4 py-6 z-40">
      <Link href={ROUTES.CLIENTE_DASHBOARD} className="flex items-center gap-3 px-2 mb-8">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-zinc-500 uppercase tracking-wider mt-1">Mi cuenta</p>
        </div>
      </Link>
      ...
      <div className="mt-auto flex items-center gap-3 border-t border-white/10 pt-4 px-2">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-medium text-zinc-200 truncate">{fullName ?? "Cliente"}</p>
        </div>
