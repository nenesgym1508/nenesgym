...
import { createTrainingRoutineAction } from "@/actions/training-routines.actions"
...
interface NuevaRutinaBibliotecaFlowProps {
  returnToDate?: string
}

export function NuevaRutinaBibliotecaFlow({ returnToDate }: NuevaRutinaBibliotecaFlowProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    const res = await createTrainingRoutineAction({
    ...
    if (res.success && res.id) {
      const suffix = returnToDate ? `?schedule=1&date=${returnToDate}` : ""
      router.push(`${adminRutinaBibliotecaDetalle(res.id)}${suffix}`)
    } else {
