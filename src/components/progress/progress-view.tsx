...
// Vista de "Progreso" compartida entre el cliente (su propia vista) y el admin
// (viendo/editando el progreso de un cliente). El comportamiento (qué acción
// de servidor se llama al guardar) se decide con `clientId`: ausente = el
// cliente edita lo suyo; presente = el admin edita en nombre de ese cliente
// (ver adminSaveProgressRecordAction/adminSetProgressGoalAction).
...
interface ProgressViewProps {
  ...
  /** Fechas ("yyyy-MM-dd") con asistencia del mes en curso, para el contador y la racha. */
  monthlyAttendanceDates: string[]
  today: string
  /** Presente = vista admin editando a este cliente; ausente = el cliente edita lo suyo. */
  clientId?: string
}

export function ProgressView({ records, goal, monthlyAttendanceDates, today, clientId }: ProgressViewProps) {
  const todayRecord = records.find((r) => r.measured_date === today) ?? null
  const attendanceDates = new Set(monthlyAttendanceDates)
  ...
  const daysSinceLast = latest && !todayRecord
    ? differenceInDays(new Date(), parseISO(latest.measured_date ?? latest.recorded_at))
    : null
    ...
    <ProgresoTabs
      progresoContent={
        <div className="space-y-6">
          <GoalCard goal={goal} clientId={clientId} />
          <ProgressForm todayRecord={todayRecord} latestHeightCm={latest?.height_cm} clientId={clientId} />
